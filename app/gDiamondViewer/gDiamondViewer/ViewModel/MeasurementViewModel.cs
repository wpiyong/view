...
namespace gDiamondViewer.ViewModel
{
    ...
    class MeasurementViewModel : ViewModelBase
    {
        ...
        public MeasurementViewModel(StatusViewModel svm, CameraViewModel cvm, SpectrometerViewModel sp)
        {
            ...
            CommandTogglePump = new RelayCommand(param => TogglePump(), cc => ArduinoConnected);
        ...
        public RelayCommand CommandTogglePump { get; set; }
        ...
        void StartLed()
        {
            ...
            {
                _cameraVM.SetMeasureMode(MeasureMode.Led);
                _cameraVM.SetShutterTime(Properties.Settings.Default.ShutterTime);
                _lightON = true;
                Application.Current.Dispatcher.BeginInvoke(new Action(System.Windows.Input.CommandManager.InvalidateRequerySuggested));
                _statusVM.Busy++;
                DateTime timestamp = DateTime.Now;
                var sts = new StatusMessage { Timestamp = timestamp, Message = "Led is ON.  Camera shutter = " + Properties.Settings.Default.ShutterTime + " ms" };
                _statusVM.Messages.Add(sts);
                _timestamps.Push(timestamp);
            }
        }
        ...
        void StartFluorescence()
        {
            ...
            {
                // fluo camera setting same as led, no need to set shutter
                ...
                //    return;
                //}
                _cameraVM.SetMeasureMode(MeasureMode.Fluorescence);
        ...
        void StartPhosphorescence()
        {
            ...
            if (result == true)
            {
                ...
                _cameraVM.SetGain(Properties.Settings.Default.GainPhos);
                int count = 5;
                bool success = false;
                while (count > 0)
                {
                    if(Math.Abs(_cameraVM.GetGain() - Properties.Settings.Default.GainPhos) < 0.01)
                    {
                        success = true;
                        break;
                    }
                ...
                if (!success)
                {
                    MessageBox.Show("Could not set phoes gain", "Camera error");
                    return;
                }

                // update the settings
                _cameraVM.Shutter = Properties.Settings.Default.ShutterTimePhos;
                _cameraVM.Gain = Properties.Settings.Default.GainPhos;
                _cameraVM.PhosDelay = Properties.Settings.Default.PhosCamDelay;
                _cameraVM.VideoMode = Properties.Settings.Default.VideoMode;

                _cameraVM.SetMeasureMode(MeasureMode.Phosphorescence);

                _spectrometerPhosResults = new SpectrometerPhosResults(0, new List<AvantesSpectrum>());
                _camPhosResults = new CamPhosResults(0, new List<PtGreyCameraImage>(), new List<System.Drawing.Bitmap>());
        ...
        void ShowPhosResults()
        {
                ...
                //&& _spectrometerPhosResults.Spectra.Count == Properties.Settings.Default.PhosSpectraCount)
            {
                ...
                var phosResultsVM = new PhosResultsViewModel(_camPhosResults, _spectrometerPhosResults, _cameraVM);
        ...
        void End()
        {
            ...
            else if (_lightON)
            {
                ...
                if (!_cameraVM.SetShutterTime(_cameraVM.Shutter))
                {
                    MessageBox.Show("Could not set shutter time", "Camera error");
                    return;
                }
