 /*Room light imaging Mode: R
Fluorescence Mode   F----E
Phorescence Mode     P*/  
int inByte = 0;         // incoming serial byte
int count = 0;
int pulse_num = 0;
int flag_delay_cnt = 0;   
int HTIME_CNT = 620; //  1000=83.23HZ;900=92.5HZ..700=120Hz;650=124.9;630=128HZ;620=131.8HZ
int pulse_gen = 0;
int flag_wait = 0;
int flag_gen = 0;
int MAX_PULSE_NUM = 100;
int FLAG_WIDTH = 500;
int FLAG_DELAY = 20;   //min = 1; 1000 = 6ms; 100=1ms;20=210us; 5 =120us;3=110us

int HTIME_CNT2 = 620; //the high time for the F command pulse    1000=83.23HZ;700=116.4Hz;650=124.7HZ;620=131.6
int FLAG_WIDTH2 = 1000; // the width for the second flag 5.4ms Pulse,camera trigger on time
int count2 = 0;
int pulse_gen2 = 0;

const int triggerPin = 12;
const int LEDPin = 11;
const int filterPin = 10;
const int SpectrometerPin = 9;
const int cameratrigger = 8;
const int pump = 7;
void setup() {   

  pinMode(triggerPin, OUTPUT);   // miniFlash trigger is on digital pin 12
  digitalWrite(triggerPin, LOW);
  pinMode(LEDPin, OUTPUT);   
  digitalWrite(LEDPin, LOW);
  pinMode(filterPin, OUTPUT);   
  digitalWrite(filterPin, LOW);
  pinMode(SpectrometerPin, OUTPUT);   
  digitalWrite(SpectrometerPin, LOW);
  pinMode(cameratrigger, OUTPUT);   // miniFlash trigger is on digital pin 8
  digitalWrite(cameratrigger, LOW);
  pinMode(pump, OUTPUT);   // miniFlash trigger is on digital pin 8
...
void loop() {
    if (Serial.available() > 0) {   
        ...
        if (inByte == 'F'){
            ...
            digitalWrite(SpectrometerPin, HIGH);
            Serial.println('A');
        }
        
        if ((inByte == 'P') && (pulse_gen2 == 0)) {
            pulse_gen = 1;
            digitalWrite(triggerPin, HIGH);
            Serial.println('A');
        }
        ...
        else if (inByte == 'S'){// vaccum pump on when it's low
            digitalWrite(pump,LOW);
            Serial.println('A');
        }   
        else if (inByte == 'V')           
            Serial.println("H_flashlamp4.ino");     
       }
     
     if (pulse_gen2){ // F command
         count2++;
         
         if (count2 == HTIME_CNT2){
             digitalWrite(triggerPin, LOW);
         }
         else if (count2 == 2*HTIME_CNT2){
             digitalWrite(triggerPin, HIGH);
             count2 = 0;
         }

         if (count2 == FLAG_WIDTH2)
            {
            digitalWrite(cameratrigger, LOW);
            digitalWrite(SpectrometerPin, LOW);
            }
     ...
     if (pulse_gen){
         if (pulse_num < MAX_PULSE_NUM){
                count++;
                if (count == HTIME_CNT){
                    digitalWrite(triggerPin, LOW);
                    pulse_num++;
                }
                else if (count == 2*HTIME_CNT){
                    digitalWrite(triggerPin, HIGH); 
                    count = 0;
                    
                    if (pulse_num == MAX_PULSE_NUM-1){
                        flag_wait = 1;
                        flag_delay_cnt = 0;
                    }
                }
